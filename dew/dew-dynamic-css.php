...
class Dew_Dynamic_CSS extends Dew_Theme_Base {
	...
	public function __construct() {
		...
		$opt_name = dew_suite()->get_option_name();
	 ...
	 */
	/**
	 * set file or inline mode
	 */
	public function set_mode() {

		// Check if we're using file mode or inline mode.
		$mode = dew_assist()->get_theme_option( 'dynamic_css_compiler' ) ? 'file' : 'inline';
		...
		if ( 'file' == $mode && $this->needs_update() ) {
			...
			$last_time    = (int) get_option( 'dew_dynamic_css_time' );
	...
	public function make_css() {
		...
		if ( ! $wp_filesystem->put_contents( $this->file( 'path' ), $content, FS_CHMOD_FILE ) ) {
			...
			$option  = get_option( 'dew_dynamic_css_posts', array() );
			$option[$page_id] = true;
			update_option( 'dew_dynamic_css_posts', $option );
	...
	public function needs_update() {
		...
		// Get the 'dew_dynamic_css_posts' option from the DB
		$option  = get_option( 'dew_dynamic_css_posts', array() );
		...
		// If the current page ID exists in the array of pages defined in the 'dew_dynamic_css_posts' option
	...
	/**
	 * Check if the CSS file is writable.
	 */
	public function can_write() {
		...
		$file_name   = '/dew-css' . $blog_id . '-' . $page_id . '.css';
		$folder_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'dew-styles';
	...
	public function file( $target = 'path' ) {
		...
		$css_uri = trailingslashit( $css_uri_folder ) . 'dew-styles/' . $file_name;
	...
	public function dynamic_css_cached() {
		 ...
		 */
		$cache = ( $cache && ( null == dew_assist()->get_theme_option( 'dynamic_css_db_caching' ) || ! dew_assist()->get_theme_option( 'dynamic_css_db_caching' ) ) ) ? false : $cache;
		...
		if( $cache ) {
			...
			/**
		 	 ...
		 	 */
			$transient_name = $page_id ? 'dew_dynamic_css_' . $page_id : 'dew_dynamic_css_global';
	...
	public function dynamic_css() {
		 ...
		 */
		$dynamic_css = $this->dynamic_css_parser( dew_dynamic_css_array() );

		/**
		 * Append the user-entered dynamic CSS
		 */
		$dynamic_css .= wp_strip_all_tags( dew_assist()->get_theme_option( 'custom_css' ) );
	...
	public function dynamic_css_parser( $css ) {
		...
		return apply_filters( 'dew_dynamic_css', $final_css );
	}
	...
	public function enqueue_dynamic_css() {
		...
		if ( 'file' == self::$mode ) {
			...
			wp_enqueue_style( 'dew-dynamic-css', $this->file( 'uri' ) );
		}
	}
	...
	public function add_inline_css() {
		...
		if ( ('inline' == self::$mode || $wp_customize) && $dynamic_css = $this->dynamic_css_cached() ) {
			echo "<style id='dew-stylesheet-inline-css' type='text/css'>" . $dynamic_css . '</style>';
		}
	...
	public function reset_post_transient( $post_id ) {
		delete_transient( 'dew_dynamic_css_' . $post_id );
	}

	/**
	 * Update css option when post saved.
	 */
	...
	public function add_options() {
		/*
		 * The 'dew_dynamic_css_posts' option will hold an array of posts that have had their css generated.
		 * We can use that to keep track of which pages need their CSS to be recreated and which don't.
		 */
		add_option( 'dew_dynamic_css_time', array(), '', 'yes' );
		/**
		 * The 'dew_dynamic_css_time' option holds the time the file writer was last used.
		 */
		add_option( 'dew_dynamic_css_time', time(), '', 'yes' );
	}
	...
	public function reset_all_transients() {
		...
		$sql = "DELETE FROM $wpdb->options WHERE option_name LIKE '_transient_dew_dynamic_css_%'";
		$wpdb->query( $sql );
	}
	...
	public function clear_cache() {
		...
		if ( dew_assist()->get_theme_option( 'dynamic_css_compiler' ) && dew_assist()->get_theme_option( 'cache_server_ip' ) ) {
	...
	public function clear_varnish_cache( $url ) {
		...
		if ( dew_assist()->get_theme_option( 'cache_server_ip' ) ) {
			$varniship = dew_assist()->get_theme_option( 'cache_server_ip' );
	...
	/*
	 * Update the dew_dynamic_css_posts option when the theme options are saved.
	 * This basically empties the array of page IDS from the 'dew_dynamic_css_posts' option
	 */
	/**
	 * reset css option
	 */
	public function global_reset_option() {
		update_option( 'dew_dynamic_css_posts', array() );
	}
	...
	public function update_saved_time() {
		update_option( 'dew_dynamic_css_time', time() );
	}
}
