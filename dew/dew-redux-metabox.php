...
class Dew_Redux_Metabox extends Dew_Theme_Base {
	...
	public function __construct() {
		...
		if ( dew_suite()->get_option_name() ) {
			$this -> add_action( 'redux/metaboxes/'.dew_suite()->get_option_name().'/boxes', 'add_metaboxes' );
		}
	}
	...
	public function add_metaboxes() {
		...
		$available_sections = get_theme_support( 'dew-metaboxes' );
		...
		foreach( $available_sections as $section ) {
			$file = "metaboxes/dew-{$section}.php";
		...
		foreach ( $sections as $section ) {
			...
			foreach ( $section['post_types'] as $post_type ) {
				...
				if ( isset( $section['separate_box'] ) && $section['separate_box'] === true ) {
					$metabox_id = 'dew-'.$post_type.'-'.sanitize_key($section['box_title']).'-options';
					$box_title = $section['box_title'];
				} else {
					$metabox_id = 'dew-'.$post_type.'-options';
					$box_title = sprintf( esc_html__('%s Options', 'voltroid-theme'), ucwords(str_replace( array('_','-'), ' ', $post_type ) ) );
				}
